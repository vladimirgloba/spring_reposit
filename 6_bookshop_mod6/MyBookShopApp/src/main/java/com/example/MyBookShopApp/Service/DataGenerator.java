...
EntityManagerRepository em;
    ...
    private Book2UserEntity generatorForOneEntity(Faker faker) {
        ...
        book2UserEntity.setUserId(Integer.parseInt(faker.number().digits(5)));
        book2UserEntity.setTime(faker.date().between(datePrevious, dateNow)
                .toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime());
        book2UserEntity.setTypeId(faker.number().numberBetween(1, 4));
        return book2UserEntity;
    }

    public List<Book2UserEntity> generatorSqlString() {
        ...
        String sqlPart1 = "insert into book2user (id, time, type_Id, book_id, user_id) values \n";
        String sqlPart2 = "";
        for (Book2UserEntity book2UserEntity : book2UserEntityList) {
            Timestamp timestamp = Timestamp.valueOf(book2UserEntity.getTime());
            sqlPart2 = sqlPart2 + "(" + book2UserEntity.getId() + ", '" + timestamp + "', " +
                    book2UserEntity.getTypeId() + ", " + book2UserEntity.getUserId() + ", " + book2UserEntity.getUserId() + "), \n";
        }
        sqlPart1 = sqlPart1 + sqlPart2.substring(0, sqlPart2.length() - 2);
        System.out.println(book2UserEntityList.size());
    ...
    public void genreGenerator() {
        ...
        idParent.forEach(System.out::println);
    }
    ...
    public List<GenreEntity> allGenres() {
        ...
        System.out.println("============second+++++++++++++" + entities.size());
        entities.stream().map(entity -> "id = " + entity.getId() + "; name = " + entity.getName() + "; parentId = " + entity.getParentId()).forEach(System.out::println);
        return entities;
    }
